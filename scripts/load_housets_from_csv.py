...
def load_housets_csv(file_path: str, dc_zip_codes: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    Args:
        file_path: HouseTS.csv 檔案路徑
        dc_zip_codes: DC ZIP Code 列表（可選，如果提供則只載入這些 ZIP Code）
    
    Returns:
        DC 地區的 HouseTS 資料 DataFrame
    """
...
def extract_latest_census_data(housets_df: pd.DataFrame) -> Dict[str, Dict]:
    ...
    根據論文，Census 資料是年度更新的，我們需要：
    1. 找出每個 ZIP Code 最新的 Census 資料
    2. 提取 Census 相關欄位
    
    Args:
        housets_df: HouseTS DataFrame（已篩選 DC 地區）
    
    Returns:
        字典，key 為 ZIP Code，value 為最新的 Census 資料
    """
    print(f"\n提取最新的 Census 資料...")
    ...
    for _, row in latest_data.iterrows():
        ...
        poi_fields = {
            ...
            'school_poi': 'school',  # 避免與 Census 的 school_enrollment 混淆
            'station': 'station',
...
if __name__ == "__main__":
    # 測試
    print("測試 HouseTS CSV 載入...")
    
    # 讀取我們現有的 ZIP Codes
    crime_df = pd.read_csv('DC_Crime_Incidents_2025_08_09_with_zipcode.csv')
    dc_zip_codes = crime_df[crime_df['ZIP_CODE'].notna()]['ZIP_CODE'].astype(str).unique().tolist()
    ...
    if not housets_df.empty:
        ...
        print(f"\nCensus 資料摘要:")
        print(f"  總 ZIP Code 數: {summary.get('total_zip_codes', 0)}")
        print(f"  有人口資料的 ZIP Code: {summary.get('zip_codes_with_population', 0)}")
        
        # 顯示範例
        if census_dict:
            sample_zip = list(census_dict.keys())[0]
            print(f"\n範例 ZIP Code ({sample_zip}):")
            sample_data = census_dict[sample_zip]
